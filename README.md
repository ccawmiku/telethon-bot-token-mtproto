...
带网页控制面板的 Telegram 媒体下载 bot。容器启动后先打开 Web 面板填写 `API_ID`、`API_HASH`、`BOT_TOKEN`，再启动 bot。你把图片、视频或文件发给 bot 后，bot 会在 Telegram 里返回排队、下载进度、完成或失败状态，同时控制面板会显示下载记录和文件列表。
...
- Telegram 消息内实时返回下载状态。
- 控制面板显示 bot 状态、下载记录、进度、错误和已保存文件。
