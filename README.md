...
- Web 下载记录和文件列表增加图片、视频缩略预览；点击预览可下载原文件。
- 图片预览使用 Pillow 按需缩放，视频预览使用 FFmpeg 截取画面，并缓存到配置目录。
- Web 增加“重试全部失败”操作，将全部可重试的失败、中断和取消任务重新加入有界队列。
- 下载记录将预览固定在状态前，增加行高，并重新分配文件名、进度、传输和时间列宽。
- 控制台扩展到宽屏布局，设置栏收窄，标签页与批量操作分组，窄屏时自动切换单栏。
- 页面显示当前版本，首页禁用缓存，JavaScript 地址携带版本参数，避免升级后继续使用旧前端。
...
```text
ghcr.io/ccawmiku/telethon-bot-token-mtproto:v1.8
```
